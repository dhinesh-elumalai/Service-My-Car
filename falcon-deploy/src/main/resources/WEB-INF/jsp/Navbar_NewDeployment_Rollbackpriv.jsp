...
<div id="page-container" class="sidebar-o side-scroll page-header-fixed main-content-boxed">
			...
			<div class="block block-bordered">
				<div class="block-header block-header-default">
					<h3 class="block-title font-w600">
						DEPLOYMENT - ${repoName}<br><small style="color: #e86e6e">Select the <b>rollback scripts</b> for this deployment request. These scripts will not be deployed.<br> They will be available in the request management page just in case you need to rollback this deployment.</small>
					</h3>
					<div class="block-options xs-text-right">
					<form action="${pageContext.request.contextPath}/PreviousRollback_step2" method="post">
				    ...
				    <button  class="btn btn-primary" type="button" id="enablenext" onclick="submitNextForm()"  data-toggle="click-ripple">Next</button>
					...
					<form action="${pageContext.request.contextPath}/submitRollbackFile_step3" method="post" id="nextForm">
					<input type="hidden" name="reponame" value='${repoName}'>
					<input type="hidden" name="reqid" value='${request.deployRequestId}'>
					<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" />
				...
				<div class="block-content">
					<div class="form-group row">
						<div class="col-md-4">
							<div class="">
							<label for="material-select2">Select Rollback Script Input Type</label>
								<select class="form-control mySelect section-changer" id="scriptType" name="type">
									<option value="select-box" <c:if test = '${request.getRollbackfileSelectionBox()=="select-box"}'> selected</c:if>>Select Box</option>
									<option value="text-box" <c:if test = '${request.getRollbackfileSelectionBox()=="text-box"}'> selected</c:if>>Text Box</option>
								</select> 
							</div>
						</div>
					</div>
							...
							<div class="row">
								<div class="block-header width-100">
									...
									<button type="button" class="btn btn-primary"
										...
										Validate selected rollback scripts</button>
								</div>
							</div>
				...
				<div class="">
					<div class="block block-borderd block-header">
						...
						<div class="row box-changer col-lg-12" id="text-box">
							<div class="">
								<textarea rows="10" cols="100" class="area-change col-12" name="textArea"placeholder="Type File name with Comma (,) or (new line) as separator." id="textAreaBox"> <c:forEach items="${request.getDeployRequestRepoFileMap()}" var='files'> <c:if test="${files.rollback}">${fn:replace(files.fileName, '\\\\','\\')}&#13;&#10;</c:if></c:forEach></textarea>
							</div>
						</div>
					</div>
	...
	<%@include file="/static/js/pages/Navbar_NewDeployment_rollback.jsp"%>
</body>
</html>

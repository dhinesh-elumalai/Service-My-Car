...
<div id="page-container" class="sidebar-o side-scroll page-header-fixed main-content-boxed">
				...
				<div class="block block-bordered">
					...
					<div class="" id="fixedSet">
							...
							<div class="block-options xs-pl-0">
								<div class="">
									<sec:authorize access="hasAnyAuthority('ADMIN','SUPER_ADMIN')">
										<a href="${pageContext.request.contextPath}/newuser" class=" btn btn-primary" data-toggle="click-ripple"><i
											class="fa fa-user"></i>+ New User</a>
									</sec:authorize>
								</div>
							</div>
						</div>
						...
						<div class="no-data text-center" id="emptyDataajax" style="display: none;">
									<p>There is no data to display.<sec:authorize access="hasAnyAuthority('ADMIN','SUPER_ADMIN')"> please <a href="${pageContext.request.contextPath}/newuser">Click here</a> to add.</sec:authorize></p>
							</div>
										...
										<c:forEach var="user" items="${page.content}">
											<tr>
												<td align="right" class="text-center">
													<img class="img-thumbnail"  id="img__${user.getId()}" src="image/${user.getId()}" alt="" style="height: 54px;width: 54px;" />
												</td>
												...
												<c:forEach items="${user.userRole}" var="Role">
													...
													<c:if test="${Role.role=='SUPER_ADMIN'}"><span class="badge badge-primary">${Role.role}</span></c:if>
													<c:if test="${Role.role=='ADMIN'}"><span class="badge badge-info" >${Role.role}</span></c:if>
													<c:if test="${Role.role=='TEAM_ADMIN'}"><span class="badge badge-warning" style="background-color:#f1a564;">${Role.role}</span></c:if>
													<c:if test="${Role.role=='USER'}"><span class="badge badge-success">${Role.role}</span></c:if>
												...
												<c:choose>
											    	<c:when test="${user.enabled}">
												        <span class="badge badge-info" style="background-color:#44ab4c">Active</span>
												    </c:when>
												    <c:otherwise>
												...
												<sec:authorize access="hasAnyAuthority('TEAM_ADMIN')">
													<c:forEach items="${user.userRole}" var="Role">
														<c:choose>
															<c:when
																test="${(Role.role=='ADMIN')||(Role.role=='SUPER_ADMIN')||(Role.role=='TEAM_ADMIN')}">
																<td></td>
															</c:when>

															<c:otherwise>
																<td class="text-center">
																	<form action="${pageContext.request.contextPath}/update-users" method="post">
																		<input type="hidden" value="${user.id}" name="id">
																		<input type="hidden" name="${_csrf.parameterName}"
																			value="${_csrf.token}" />
																		<button class="btn-none" id="submit">
														...
														<c:choose>
															...
															<c:otherwise>
																<td class="text-center"><c:if
																		test="${(Role.users.email!=userNameDisplay.email)}">
																		<form action="${pageContext.request.contextPath}/update-users" method="post">
																			<input type="hidden" value="${user.id}" name="id">
																			<input type="hidden" name="${_csrf.parameterName}"
																				value="${_csrf.token}" />
																			<button id="submit" class="btn-none">
																				<span class="fa fa-pencil-square-o"
																					style="font-size: 18px;"></span>
																			</button>
																		</form>
	...
	<%@include file="/static/js/pages/usersJS.jsp"%>

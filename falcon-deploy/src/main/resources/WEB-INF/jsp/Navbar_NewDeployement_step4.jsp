		...
		<div class="content">
			<div class="block block-bordered">
				<div class="block-header block-header-default">
					<h3 class="block-title">DEPLOYMENT - ${repoName}</h3>
					<div class="block-options xs-text-right">
					
				<form action="${pageContext.request.contextPath}/updateDeployRequestDB" method="post">
					...
					<button type="button" class="btn btn-primary" onclick="submitForm()" data-toggle="click-ripple">Next</button>
				...
				<form method="post" action="${pageContext.request.contextPath}/submitStep4" id="scheduledForm">
							...
							<div class="form-group row">
								<div class="col-12">
									<div class="custom-control custom-radio mb-20">
										<input class="custom-control-input" type="radio" name="schedule" id="example-radio1" value="true"
											<c:choose>
                                                <c:when test="${sceduler!=null}">
                                                 <c:if test="${sceduler.isOnceImmediately()}">checked</c:if>
                                                  </c:when>
                                                  <c:otherwise>
                                                  checked
                                                  </c:otherwise>
											...
											for="example-radio1">Once, Immediately</label>
									</div>
									<div class="custom-control custom-radio mb-20">
										<input class="custom-control-input" type="radio" name="schedule" id="example-radio2" value="false"
											<c:choose>
                                                <c:when test="${sceduler!=null}">
                                                 <c:if test="${not sceduler.isOnceImmediately()}">checked</c:if>
                                                  </c:when>
                                                </c:choose>
											> <label
											class="custom-control-label" for="example-radio2">Once,
											Later</label>
									</div>
									<div id="date-sec" 	<c:choose>
                                                <c:when test="${sceduler!=null}">
                                                 <c:if test="${ sceduler.isOnceImmediately()}">style="display: none"</c:if>
                                                  </c:when>
                                                   <c:otherwise>
                                                         style="display: none"
                                                 </c:otherwise>
                                                ...
                                                <jsp:setProperty name="dateValue125" property="time" value="${sceduler.getScheduledTimestamp().getTime()}" />
                                                 <jsp:useBean id="dateValue126" class="java.util.Date" /> 
                                                <jsp:setProperty name="dateValue126" property="time" value="${currentDateApp.getTime()}" />
														<div class="form-group  row">
													<div class="input-group ">
														<label class="col-md-2 control-label text-right" for="time">Timezone:</label>
														<div class="col-md-4">									
<select id="timezone" name="selectedTZ" class="js-select2 form-control width-100 select-validation col-md-9" data-placeholder="Select your timezone" required>
		<option></option>  
...
<option value="Pacific/Yap" label="Yap">(GMT+10) Pacific/Yap</option>
										...
										<div class="form-group row">										
											<div class="input-group">
											<label class="col-md-2 control-label text-right">Date:</label>
												<div class="col-md-4">	
	                                             <input id="date" class="form-control" name="date" data-provide="datepicker" <c:if test="${sceduler==null}">value='<fmt:formatDate value="${dateValue126}"  pattern="MM-dd-yyyy"/>'</c:if> <c:if test="${sceduler!=null}">
	                                           <c:choose>
	                                             <c:when test="${sceduler.isOnceImmediately()}">
	                                              value='<fmt:formatDate value="${dateValue126}"  pattern="MM-dd-yyyy"/>'
	                                             </c:when>
	                                              <c:otherwise>
	                                              value='<fmt:formatDate value="${dateValue125}"  pattern="MM-dd-yyyy"/>'
	                                             </c:otherwise>
	                                             </c:choose>
										...
										<div class="form-group row">
												<div class="input-group ">
												<label class="col-md-2 control-label text-right">Time:</label>
													<div class="col-md-4">	
													<input type="text" class="form-control timepicker" name="time"  data-minute-step="1" 
													<c:if test="${sceduler!=null}">
													 <c:choose>
	                                             <c:when test="${sceduler.isOnceImmediately()}">
	                                              value='<fmt:formatDate value="${dateValue126}"  pattern="hh:mm aaa"/>'
	                                             </c:when>
	                                              <c:otherwise>
	                                              value='<fmt:formatDate value="${dateValue125}"  pattern="hh:mm aaa"/>'
	                                             </c:otherwise>
	                                             </c:choose> 
	                                             </c:if>
													<c:if test="${sceduler==null}">value='<fmt:formatDate value="${dateValue126}"  pattern="hh:mm aaa"/>'</c:if> autocomplete="off" > 
												    <span class="input-group-addon cal-icon1" ><i class="fa fa-clock-o"></i></span>
												</div>	
												</div>							
										</div>
									</div>
				...
				<div class="col-md-12 mt-15">
					<div class="block block-bordered ">
						<div class="block-header block-header-default">
							<h3 class="block-title">E-mail Notification</h3>
						</div>
						<div class="block-content">
						<div class="form-group row">
                            <div class="col-12">
                                <div class="custom-control custom-checkbox mb-20">
                                    <input class="custom-control-input" type="checkbox" name="success" id="success" value="true"<c:if test="${sceduler.getDeployRequest().mailOnsuccess}">checked</c:if> >
                                    <label class="custom-control-label" for="success">On success</label>
                                </div>
                                <div class="custom-control custom-checkbox mb-20">
                                    <input class="custom-control-input" type="checkbox" name="failed" id="failed" value="true"<c:if test="${sceduler.getDeployRequest().mailOnfail}">checked</c:if>>
                                    <label class="custom-control-label" for="failed">On failure</label>
                                </div>
	...
	<%@include file="/static/js/pages/Navbar_NewDeployement_step4JS.jsp"%>
</body>
</html>
